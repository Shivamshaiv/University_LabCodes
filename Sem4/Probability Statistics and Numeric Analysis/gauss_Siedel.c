...
{
     *(prevx)=(B[0][3]-(*prevy) *B[0][1]-(*prevz) *B[0][2])/B[0][0];
     *(prevy)=(B[1][3]-(*prevx) *B[1][0]-(*prevz)* B[1][2])/B[1][1];
     *(prevz)=(B[2][3]-(*prevx) *B[2][0]-(*prevy)* B[2][1])/B[2][2];
}
...
{   srand(time(NULL));
	...
	printf("\n The check answer is : %f", checkans);
}
